...
- 由于 `RVC` 目标说话人性别本身可能干扰主观判断，当前人工听审已临时回切为“先只看模块本体修正前后”
- 共用 GUI 仍可读取 `stage1 cascade` 队列，但当前默认口径会先忽略 `RVC` 输出，不把它作为本轮第一判断依据

因此当前下一步调整为：

1. 先按模块本体 `source vs preconditioned` 做主观打分与导出
2. 等模块本体结论稳定后，再恢复 `raw vs preconditioned -> RVC` 的 cascade 判断
3. 再决定这些“单独听起来几乎无感”的前置器，是否在串联后真的有帮助
