...
def build_readme(args: argparse.Namespace, output_dir: Path, rows: int) -> str:
    ...
    return (
        ...
        f"- latent_cap: `{args.latent_cap:.3f}`\n"
...
def build_summary(rows: list[dict[str, str]], args: argparse.Namespace, device: str, sample_rate: int) -> str:
    ...
    weakest = sorted(rows, key=lambda row: float(row["final_mel_loss"]))[:3]
    lines = [
        "# Encodec ATRR Latent Probe Summary v1",
        "",
        f"- rows: `{len(rows)}`",
    ...
    for row in strongest:
        lines.append(
            ...
            f"mel_loss=`{row['final_mel_loss']}`"
        )
    lines.extend(["", "## Lowest Mel Loss Rows", ""])
    for row in weakest:
        lines.append(
            f"- `{row['record_id']}` | mel_loss=`{row['final_mel_loss']}` | "
            f"wave_l1=`{row['final_wave_l1']}`"
        )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    for row in rows:
        ...
        if args.voiced_only:
            ...
            latent_time_gate_np = align_mask(voiced_mask, int(base_latent.shape[-1])).astype(np.float32)
        else:
            latent_time_gate_np = np.ones((int(base_latent.shape[-1]),), dtype=np.float32)
        ...
        output_path = processed_dir / f"{record_id}__encodec_latent_bw{int(args.bandwidth)}.wav"
    ...
    summary_csv = output_dir / "encodec_atrr_latent_probe_summary.csv"
    summary_md = output_dir / "ENCODEC_ATRR_LATENT_PROBE_SUMMARY.md"
