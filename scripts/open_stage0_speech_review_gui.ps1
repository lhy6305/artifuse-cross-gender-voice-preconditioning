...
$packBuildScript = Join-Path $PSScriptRoot "build_stage0_speech_listening_pack.py"
...
$ruleConfig = Join-Path $repoRoot "experiments\stage0_baseline\v1_full\speech_rule_candidate_v1.json"
$packDir = Join-Path $repoRoot "artifacts\listening_review\stage0_speech_listening_pack\v1"
...
Write-Host "Using speech listening pack:" $packDir
...
if (-not (Test-Path $summaryCsv)) {
  Write-Host "Building speech listening pack..."
  Invoke-Python -Arguments @($packBuildScript)
}

if (-not (Test-Path $summaryCsv)) {
  throw "Missing speech listening pack summary after build: $summaryCsv"
}

$needQueue = $Rebuild -or (-not (Test-Path $queueCsv))
if ($needQueue) {
  Write-Host "Building speech listening review queue..."
  ...
  Write-Host "Opening cached speech listening review queue..."
}

if (-not (Test-Path $queueCsv)) {
  throw "Missing speech listening review queue: $queueCsv"
}
