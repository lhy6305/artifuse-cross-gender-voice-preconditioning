...
DEFAULT_OUTPUT_DIR = (
    ...
    / "artifacts"
    / "diagnostics"
    / "encodec_atrr_code_refit_probe"
    / "v1_fixed8_q24_n8_teacherlatentv1_bw24"
)
...
def build_readme(args: argparse.Namespace, output_dir: Path, rows: int, total_quantizers: int) -> str:
    ...
    return (
        ...
        f"- teacher_lr: `{args.teacher_lr:.4f}`\n"
        f"- teacher_init_scale: `{args.teacher_init_scale:.4f}`\n"
        f"- teacher_latent_cap: `{args.teacher_latent_cap:.3f}`\n"
        f"- lambda_latent_l2: `{args.lambda_latent_l2:.3f}`\n"
        f"- lambda_latent_time: `{args.lambda_latent_time:.3f}`\n"
...
def build_summary(
    ...
    changed = sorted(rows, key=lambda row: float(row["changed_code_ratio"]), reverse=True)[:3]
    active_stop = min(int(args.quantizer_start) + int(args.quantizer_count), int(total_quantizers))
    lines = [
    ...
    lines.extend(["", "## Highest Changed-Code Rows", ""])
    for row in changed:
        lines.append(
            f"- `{row['record_id']}` | changed_code_ratio=`{row['changed_code_ratio']}` | "
            f"projection_l1=`{row['teacher_projection_l1']}`"
        )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    for row in rows:
        ...
        with torch.no_grad():
            active_latent = torch.zeros_like(base_latent)
            ...
            for layer_idx in active_quantizers:
                ...
                proposed_latent = layer.decode(proposed_indices)
                base_indices = q_indices[layer_idx]
                base_layer = base_layer_latents[layer_idx]
                if args.voiced_only:
                    proposed_latent = torch.where(gate_bool, proposed_latent, base_layer)
            ...
            refit_latent = inactive_latent + active_latent
            refit_wave = decode_wave(model, refit_latent, scale, audio_length).detach()

        output_path = processed_dir / f"{record_id}__encodec_code_refit_bw{int(args.bandwidth)}.wav"
        torchaudio.save(str(output_path), refit_wave.squeeze(0).cpu(), sample_rate=int(model.sample_rate))
        ...
        summary_rows.append(
            {
                ...
                "teacher_projection_rms": f"{float(torch.sqrt(torch.mean((refit_latent - teacher_latent).square())).cpu()):.6f}",
                "teacher_projection_l1": f"{float(torch.mean(torch.abs(refit_latent - teacher_latent)).cpu()):.6f}",
                "changed_code_ratio": f"{(changed_positions / max(total_positions, 1.0)):.6f}",
    ...
    summary_csv = output_dir / "encodec_atrr_code_refit_probe_summary.csv"
    summary_md = output_dir / "ENCODEC_ATRR_CODE_REFIT_PROBE_SUMMARY.md"
