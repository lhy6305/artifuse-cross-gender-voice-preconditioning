...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--gain-cap-db", type=float, default=2.00)
    parser.add_argument("--gain-floor-db", type=float, default=-2.00)
...
def build_summary(rows: list[dict[str, str]], args: argparse.Namespace, device: str, sample_rate: int) -> str:
    ...
    strongest = sorted(rows, key=lambda row: float(row["target_logmel_delta_l1_db"]), reverse=True)[:3]
    weakest = sorted(rows, key=lambda row: float(row["applied_gain_abs_mean_db"]))[:3]
    lines = [
        "# Encodec ATRR Residual Probe Summary v1",
        "",
        f"- rows: `{len(rows)}`",
        ...
        f"- avg applied_gain_abs_mean_db: `{avg('applied_gain_abs_mean_db'):.4f}`",
        f"- avg applied_gain_abs_max_db: `{avg('applied_gain_abs_max_db'):.4f}`",
    ...
    for row in strongest:
        lines.append(
            ...
            f"gain_mean_db=`{row['applied_gain_abs_mean_db']}`"
        )
    lines.extend(["", "## Weakest Applied Rows", ""])
    for row in weakest:
        lines.append(
            f"- `{row['record_id']}` | gain_mean_db=`{row['applied_gain_abs_mean_db']}` | "
            f"gain_max_db=`{row['applied_gain_abs_max_db']}`"
        )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    for row in rows:
        ...
        delta_log_mel = np.clip(delta_log_mel * float(args.delta_scale), -float(args.delta_cap_db), float(args.delta_cap_db))

        candidate_log_mel = source_log_mel_rt + delta_log_mel
        candidate_mel_power = np.exp(candidate_log_mel)
        candidate_mag = librosa.feature.inverse.mel_to_stft(
            candidate_mel_power,
            sr=int(model.sample_rate),
            n_fft=args.n_fft,
            power=2.0,
        ...
        candidate_mag = align_columns(candidate_mag, frame_count)

        gain = candidate_mag / np.maximum(source_mag, 1e-8)
        gain_db = 20.0 * np.log10(np.maximum(gain, 1e-8))
        gain_db = np.clip(gain_db, float(args.gain_floor_db), float(args.gain_cap_db))
        gain_db = smooth_time(gain_db, int(args.time_smooth_frames))
        applied_gain = np.power(10.0, gain_db / 20.0)
        ...
        output_path = processed_dir / f"{record_id}__encodec_atrr_bw{int(args.bandwidth)}.wav"
        ...
        summary_rows.append(
            {
                ...
                "applied_gain_abs_mean_db": f"{float(np.mean(np.abs(gain_db))):.6f}",
                "applied_gain_abs_max_db": f"{float(np.max(np.abs(gain_db))):.6f}",
                "core_mask_ratio": f"{float(core_mask_ratio):.6f}",
            }
        )

    queue_path = output_dir / "listening_review_queue.csv"
    summary_csv = output_dir / "encodec_atrr_residual_probe_summary.csv"
    summary_md = output_dir / "ENCODEC_ATRR_RESIDUAL_PROBE_SUMMARY.md"
