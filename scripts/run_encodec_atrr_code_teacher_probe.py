...
DEFAULT_OUTPUT_DIR = (
    ...
    / "artifacts"
    / "diagnostics"
    / "encodec_atrr_code_teacher_probe"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--soft-rank", type=int, default=4)
    parser.add_argument("--soft-steps", type=int, default=30)
    parser.add_argument("--soft-lr", type=float, default=0.01)
    parser.add_argument("--soft-init-scale", type=float, default=0.01)
    parser.add_argument("--identity-bias", type=float, default=2.5)
    ...
    parser.add_argument("--lambda-teacher-proj", type=float, default=0.50)
    parser.add_argument("--lambda-code-l2", type=float, default=0.10)
    parser.add_argument("--lambda-code-time", type=float, default=0.05)
    parser.add_argument("--lambda-nonbase", type=float, default=0.02)
...
def build_readme(args: argparse.Namespace, output_dir: Path, rows: int, total_quantizers: int) -> str:
    ...
    return (
        ...
        f"- candidate_count: `{int(args.candidate_count)}`\n"
        f"- teacher_rank: `{int(args.teacher_rank)}`\n"
        f"- teacher_steps: `{int(args.teacher_steps)}`\n"
        f"- soft_rank: `{int(args.soft_rank)}`\n"
        f"- soft_steps: `{int(args.soft_steps)}`\n"
        f"- identity_bias: `{args.identity_bias:.2f}`\n"
        f"- temperature: `{args.temperature:.2f}`\n"
        f"- lambda_teacher_proj: `{args.lambda_teacher_proj:.3f}`\n"
        ...
        f"  --candidate-count {int(args.candidate_count)} `\n"
        ...
        f"  --soft-rank {int(args.soft_rank)} `\n"
        f"  --soft-steps {int(args.soft_steps)} `\n"
        f"  --soft-lr {args.soft_lr:.4f} `\n"
        f"  --soft-init-scale {args.soft_init_scale:.4f} `\n"
        ...
        f"  --lambda-teacher-proj {args.lambda_teacher_proj:.3f} `\n"
...
def build_summary(
    ...
    strongest = sorted(rows, key=lambda row: float(row["teacher_projection_rms"]), reverse=True)[:3]
    most_changed = sorted(rows, key=lambda row: float(row["nonbase_mass"]), reverse=True)[:3]
    active_stop = min(int(args.quantizer_start) + int(args.quantizer_count), int(total_quantizers))
    lines = [
        ...
        f"- avg teacher_projection_rms: `{avg('teacher_projection_rms'):.4f}`",
        f"- avg teacher_projection_l1: `{avg('teacher_projection_l1'):.4f}`",
    ...
    for row in strongest:
        lines.append(
            f"- `{row['record_id']}` | projection_rms=`{row['teacher_projection_rms']}` | "
            f"nonbase=`{row['nonbase_mass']}`"
        )
    lines.extend(["", "## Highest Non-Base Rows", ""])
...
def main() -> None:
    ...
    for row in rows:
        ...
        with torch.no_grad():
            for layer_idx in active_quantizers:
                ...
                candidate_index_map[layer_idx] = build_teacher_candidate_indices(
        ...
        layer_candidate_factors = torch.nn.Parameter(
            torch.randn((1, active_count * candidate_count, int(args.soft_rank)), dtype=base_latent.dtype, device=device)
            * float(args.soft_init_scale)
        )
        time_factors = torch.nn.Parameter(
            torch.randn((1, int(args.soft_rank), int(base_latent.shape[-1])), dtype=base_latent.dtype, device=device)
            * float(args.soft_init_scale)
        )
        optimizer = torch.optim.Adam([layer_candidate_factors, time_factors], lr=float(args.soft_lr))
        ...
        teacher_projection = torch.zeros_like(base_latent)
        for _ in range(max(1, int(args.soft_steps))):
            optimizer.zero_grad(set_to_none=True)
            ...
            teacher_projection = edited_latent - teacher_latent
            teacher_proj_l2 = torch.mean(teacher_projection.square())
            wave_l1 = torch.mean(torch.abs(edited_wave - base_wave))
            ...
            loss = (
                weighted_mel
                + float(args.lambda_teacher_proj) * teacher_proj_l2
        ...
        with torch.no_grad():
            ...
            for active_pos, layer_idx in enumerate(active_quantizers):
                ...
                if args.voiced_only:
                    mixed_layer = base_layer + ((mixed_layer - base_layer) * code_time_gate)
                edited_latent = edited_latent + mixed_layer
            teacher_projection = (edited_latent - teacher_latent).detach()
            edited_wave = decode_wave(model, edited_latent, scale, audio_length).detach()

        output_path = processed_dir / f"{record_id}__encodec_code_teacher_bw{int(args.bandwidth)}.wav"
        ...
        summary_rows.append(
            {
                ...
                "teacher_projection_rms": f"{float(torch.sqrt(torch.mean(teacher_projection.square())).cpu()):.6f}",
                "teacher_projection_l1": f"{float(torch.mean(torch.abs(teacher_projection)).cpu()):.6f}",
    ...
    summary_csv = output_dir / "encodec_atrr_code_teacher_probe_summary.csv"
    summary_md = output_dir / "ENCODEC_ATRR_CODE_TEACHER_PROBE_SUMMARY.md"
