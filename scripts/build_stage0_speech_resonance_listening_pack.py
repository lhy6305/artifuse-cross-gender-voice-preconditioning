...
DEFAULT_RULE_CONFIG = ROOT / "experiments" / "stage0_baseline" / "v1_full" / "speech_resonance_tilt_candidate_v1.json"
DEFAULT_INPUT_CSV = ROOT / "experiments" / "fixed_eval" / "v1_2" / "fixed_eval_review_final_v1_2.csv"
DEFAULT_OUTPUT_DIR = ROOT / "artifacts" / "listening_review" / "stage0_speech_resonance_listening_pack" / "v1"
...
def bell_gain_curve(freqs_hz: np.ndarray, bands: list[dict]) -> np.ndarray:
    ...
    for band in bands:
        ...
        width_oct = max(float(band["width_oct"]), 0.2)
        gain_db = float(band["gain_db"])
        log_distance = np.log2(safe_freqs / max(center_hz, 1.0))
        sigma = width_oct / 2.355
        curve_db += gain_db * np.exp(-(log_distance ** 2) / (2.0 * sigma ** 2))
    return curve_db


def apply_voiced_resonance_tilt(
    audio: np.ndarray,
    *,
    sample_rate: int,
    n_fft: int,
    ...
    curve_db = bell_gain_curve(freqs_hz, bands=bands)
    curve_linear = np.power(10.0, curve_db / 20.0, dtype=np.float32)
    ...
    frame_gain = np.power(curve_linear[:, None], blend * voiced_mask[None, :], dtype=np.float32)
...
def main() -> None:
    ...
    for row in selected_rows:
        ...
        stem = f"{dataset_slug}__{row['gender']}__{target_direction}__restilt__{row['utt_id']}"
        ...
        out = apply_voiced_resonance_tilt(
            audio,
