...
$packDir = Join-Path $repoRoot ("artifacts\listening_review\stage0_speech_envelope_listening_pack\{0}" -f $PackVersion)
...
Write-Host "Using speech envelope listening pack:" $packDir
...
if (-not (Test-Path $summaryCsv)) {
  throw "Missing speech envelope listening pack summary after build: $summaryCsv"
}

$needQueue = $Rebuild -or (-not (Test-Path $queueCsv))
if ($needQueue) {
  Write-Host "Building speech envelope listening review queue..."
  Invoke-Python -Arguments @(
    $queueBuildScript,
    "--rule-config", $ruleConfig,
  ...
  Write-Host "Opening cached speech envelope listening review queue..."
}

if (-not (Test-Path $queueCsv)) {
  throw "Missing speech envelope listening review queue: $queueCsv"
}
