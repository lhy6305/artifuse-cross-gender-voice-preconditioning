...
def smooth_vector(values: np.ndarray, bins: int) -> np.ndarray:
    window = max(3, int(bins))
    if window % 2 == 0:
        window += 1
...
def gaussian_curve_db(freqs_hz: np.ndarray, center_hz: float, width_oct: float, gain_db: float) -> np.ndarray:
    safe_freqs = np.maximum(freqs_hz.astype(np.float32), 1.0)
    sigma = max(width_oct, 0.2) / 2.355
    log_distance = np.log2(safe_freqs / max(center_hz, 1.0))
    return gain_db * np.exp(-(log_distance ** 2) / (2.0 * sigma ** 2))


def detect_anchor_hz(smoothed_log_mag: np.ndarray, freqs_hz: np.ndarray, low_hz: float, high_hz: float) -> float | None:
    mask = (freqs_hz >= low_hz) & (freqs_hz <= high_hz)
    if not np.any(mask):
        return None
    sub = smoothed_log_mag[mask]
    idx = int(np.argmax(sub))
    freq_candidates = freqs_hz[mask]
    return float(freq_candidates[idx])


def apply_voiced_formant_anchor_morph(
    audio: np.ndarray,
    *,
    sample_rate: int,
    n_fft: int,
    hop_length: int,
    peak_limit: float,
    anchor_ranges_hz: list[list[float]],
    shift_ratio: float,
    boost_db: float,
    cut_db: float,
    width_oct: float,
    ...
    frame_gain_db = np.zeros_like(magnitude, dtype=np.float32)

    for frame_idx in range(magnitude.shape[1]):
        if voiced_mask[frame_idx] <= 0.01:
            continue
        log_mag = np.log(np.maximum(magnitude[:, frame_idx], 1e-7))
        smoothed = smooth_vector(log_mag, smooth_bins)
        curve_db = np.zeros_like(freqs_hz, dtype=np.float32)
        for low_hz, high_hz in anchor_ranges_hz:
            anchor_hz = detect_anchor_hz(smoothed, freqs_hz, float(low_hz), float(high_hz))
            if anchor_hz is None:
                continue
            target_hz = anchor_hz * float(shift_ratio)
            curve_db += gaussian_curve_db(freqs_hz, target_hz, width_oct, boost_db)
            curve_db += gaussian_curve_db(freqs_hz, anchor_hz, width_oct, cut_db)
        curve_db = np.clip(curve_db, -abs(max_total_gain_db), abs(max_total_gain_db))
        frame_gain_db[:, frame_idx] = curve_db * blend * voiced_mask[frame_idx]

    frame_gain = np.power(10.0, frame_gain_db / 20.0, dtype=np.float32)
    adjusted_mag = magnitude * frame_gain
...
def main() -> None:
    ...
    for row in selected_rows:
        ...
        stem = f"{dataset_slug}__{row['gender']}__{target_direction}__formant__{row['utt_id']}"
        ...
        out = apply_voiced_formant_anchor_morph(
            audio,
            sample_rate=sample_rate,
            n_fft=args.n_fft,
            hop_length=args.hop_length,
            peak_limit=args.peak_limit,
            anchor_ranges_hz=params["anchor_ranges_hz"],
            shift_ratio=float(params["shift_ratio"]),
            boost_db=float(params["boost_db"]),
            cut_db=float(params["cut_db"]),
            width_oct=float(params["width_oct"]),
            blend=float(params["blend"]),
            voiced_rms_gate_db=float(params["voiced_rms_gate_db"]),
            smooth_bins=int(params["smooth_bins"]),
            max_total_gain_db=float(params["max_total_gain_db"]),
        )
