...
DEFAULT_OUTPUT_DIR = (
    ...
    / "artifacts"
    / "diagnostics"
    / "encodec_atrr_code_commit_probe"
    / "v1_fixed8_q24_n8_tr4_ts30_cr4_cs30_cb200_ct100_cl015_ctm007_cm020_w100_bw24"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--commit-rank", type=int, default=4)
    parser.add_argument("--commit-steps", type=int, default=30)
    parser.add_argument("--commit-lr", type=float, default=0.01)
    parser.add_argument("--commit-init-scale", type=float, default=0.01)
    parser.add_argument("--commit-bias", type=float, default=-2.0)
    parser.add_argument("--commit-temperature", type=float, default=1.0)
    ...
    parser.add_argument("--lambda-commit-l2", type=float, default=0.15)
    parser.add_argument("--lambda-commit-time", type=float, default=0.07)
    parser.add_argument("--lambda-commit-mass", type=float, default=0.02)
...
def build_readme(args: argparse.Namespace, output_dir: Path, rows: int, total_quantizers: int) -> str:
    ...
    return (
        ...
        f"- commit_rank: `{int(args.commit_rank)}`\n"
        f"- commit_steps: `{int(args.commit_steps)}`\n"
        f"- commit_bias: `{args.commit_bias:.2f}`\n"
        f"- commit_temperature: `{args.commit_temperature:.2f}`\n"
        f"- lambda_commit_l2: `{args.lambda_commit_l2:.3f}`\n"
        f"- lambda_commit_time: `{args.lambda_commit_time:.3f}`\n"
        f"- lambda_commit_mass: `{args.lambda_commit_mass:.3f}`\n"
        ...
        f"  --commit-rank {int(args.commit_rank)} `\n"
        f"  --commit-steps {int(args.commit_steps)} `\n"
        f"  --commit-lr {args.commit_lr:.4f} `\n"
        f"  --commit-init-scale {args.commit_init_scale:.4f} `\n"
        f"  --commit-bias {args.commit_bias:.2f} `\n"
        f"  --commit-temperature {args.commit_temperature:.2f} `\n"
        f"  --lambda-latent-l2 {args.lambda_latent_l2:.3f} `\n"
        f"  --lambda-latent-time {args.lambda_latent_time:.3f} `\n"
        f"  --lambda-commit-l2 {args.lambda_commit_l2:.3f} `\n"
        f"  --lambda-commit-time {args.lambda_commit_time:.3f} `\n"
        f"  --lambda-commit-mass {args.lambda_commit_mass:.3f} `\n"
...
def build_summary(
    ...
    most_changed = sorted(rows, key=lambda row: float(row["commit_mean"]), reverse=True)[:3]
    active_stop = min(int(args.quantizer_start) + int(args.quantizer_count), int(total_quantizers))
    lines = [
        ...
        f"- avg commit_mean: `{avg('commit_mean'):.4f}`",
    ...
    for row in strongest:
        lines.append(
            ...
            f"commit_mean=`{row['commit_mean']}`"
        )
    lines.extend(["", "## Highest Commit Rows", ""])
    for row in most_changed:
        lines.append(
            f"- `{row['record_id']}` | commit_mean=`{row['commit_mean']}` | "
...
def main() -> None:
    ...
    for row in rows:
        ...
        layer_gate_factors = torch.nn.Parameter(
            torch.randn((1, active_count, int(args.commit_rank)), dtype=base_latent.dtype, device=device)
            * float(args.commit_init_scale)
        )
        time_factors = torch.nn.Parameter(
            torch.randn((1, int(args.commit_rank), int(base_latent.shape[-1])), dtype=base_latent.dtype, device=device)
            * float(args.commit_init_scale)
        )
        optimizer = torch.optim.Adam([layer_gate_factors, time_factors], lr=float(args.commit_lr))

        code_time_gate = latent_time_gate

        final_wave_l1 = 0.0
        final_commit_mean = 0.0
        final_base_keep_mean = 1.0
        teacher_code_gap = torch.zeros_like(base_latent)
        for _ in range(max(1, int(args.commit_steps))):
            optimizer.zero_grad(set_to_none=True)
            logits = torch.matmul(layer_gate_factors, time_factors).unsqueeze(2)
            commit_probs = torch.sigmoid((logits + float(args.commit_bias)) / max(float(args.commit_temperature), 1e-4))
            hard_commit = (commit_probs >= 0.5).to(dtype=base_latent.dtype)
            commit_values = (hard_commit - commit_probs).detach() + commit_probs
            commit_values = commit_values * code_time_gate.unsqueeze(1)
            ...
            for active_pos, layer_idx in enumerate(active_quantizers):
                ...
                layer_commit = commit_values[:, active_pos, 0, :].unsqueeze(1)
                mixed_layer = base_layer + ((teacher_layer - base_layer) * layer_commit)
            ...
            commit_l2 = torch.mean(code_delta_tensor.square())
            commit_time = (
                torch.mean((commit_values[:, :, :, 1:] - commit_values[:, :, :, :-1]).square())
                if commit_values.shape[-1] > 1
            ...
            if args.voiced_only:
                ...
                commit_mean = (commit_values[:, :, 0, :] * gate).sum() / denom
                report_commit_mean = (hard_commit[:, :, 0, :] * gate).sum() / denom
            else:
                commit_mean = torch.mean(commit_values[:, :, 0, :])
                report_commit_mean = torch.mean(hard_commit[:, :, 0, :])
            base_keep_mean = 1.0 - report_commit_mean

            loss = (
                weighted_mel
                + float(args.lambda_commit_l2) * commit_l2
                + float(args.lambda_commit_time) * commit_time
                + float(args.lambda_commit_mass) * commit_mean
                + float(args.lambda_wave_l1) * wave_l1
            )
            loss.backward()
            optimizer.step()
            final_wave_l1 = float(wave_l1.detach().cpu())
            final_commit_mean = float(report_commit_mean.detach().cpu())
            final_base_keep_mean = float(base_keep_mean.detach().cpu())

        with torch.no_grad():
            logits = torch.matmul(layer_gate_factors, time_factors).unsqueeze(2)
            commit_probs = torch.sigmoid((logits + float(args.commit_bias)) / max(float(args.commit_temperature), 1e-4))
            hard_commit = (commit_probs >= 0.5).to(dtype=base_latent.dtype) * code_time_gate.unsqueeze(1)
            edited_latent = inactive_latent
            for active_pos, layer_idx in enumerate(active_quantizers):
                base_layer = base_layer_latents[layer_idx]
                teacher_layer = teacher_layer_latents[layer_idx]
                layer_commit = hard_commit[:, active_pos, 0, :].unsqueeze(1)
                mixed_layer = base_layer + ((teacher_layer - base_layer) * layer_commit)
                edited_latent = edited_latent + mixed_layer
        ...
        output_path = processed_dir / f"{record_id}__encodec_code_commit_bw{int(args.bandwidth)}.wav"
        ...
        summary_rows.append(
            {
                ...
                "commit_mean": f"{float(final_commit_mean):.6f}",
    ...
    summary_csv = output_dir / "encodec_atrr_code_commit_probe_summary.csv"
    summary_md = output_dir / "ENCODEC_ATRR_CODE_COMMIT_PROBE_SUMMARY.md"
